...
import { TextAndImageProps } from "@/slices/TextAndImage";
...
export default async function Page() {
  ...
  return <SliceZone slices={slices} components={{...components, text_and_image_bundle: ({slice}:SliceComponentProps<TextAndImageBundleSlice>) => (<div><SliceZone slices={slice.slices} components={components}/></div>)}} />;
}
...
//wrap TextAndImage Slices in a div, for animation
...
function bundleTextAndImageSlices(slices: Content.HomepageDocumentDataSlicesSlice[]){
  ...
  for(const slice of slices){
    ...
    if(bundle?.slice_type === "text_and_image_bundle"){
      ...
      res.push({id: `${slice.id}-bundle`, slice_type: "text_and_image_bundle", slices: [slice]})
    }
  }
  return res;
}
